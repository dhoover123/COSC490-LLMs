...
def create_ngrams(data, n, splitter, tokenizer):
    ...
    for context, next_words in sorted_next_word_candidates.items():
        ...
        for nw in next_words:
            score = ngrams[context + (nw,)] / ngram_context[context]
            scores.append(score)
...
def plot_next_word_prob(word_scores, word_candidates, context, top=10, save_path=None):
    probabilities = word_scores.get(context, [])
    words = word_candidates.get(context, [])
    
    # Check if either the probabilities or words are empty
    if probabilities.size == 0 or len(words) == 0:
        print(f"No valid probabilities or words found for context: {context}")
        return

    top_n = min(len(probabilities), top)
    top_indices = np.argsort(probabilities)[-top_n:][::-1]
    top_words = [words[i] for i in top_indices]
    top_probs = [probabilities[i] for i in top_indices]

    plt.figure(figsize=(10, 5))
    plt.barh(top_words, top_probs, color='skyblue')
