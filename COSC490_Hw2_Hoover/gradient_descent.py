...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    for word in word_tokenize(sentence.lower()):
        try:
            vectors.append(torch.tensor(embeddings[word],dtype=torch.float))
        except KeyError:
            pass
...
class SentimentClassifier(nn.Module):
    ...
    def gradient_loss(self, inp, logits, labels):
        ...
        one_hot_labels = torch.nn.functional.one_hot(labels, num_classes=self.linear.out_features).float()
        loss = -torch.mean(torch.sum(one_hot_labels * torch.log(softmax_probs), dim=1))
        grads_weights = torch.matmul((softmax_probs - one_hot_labels).T, inp) / bsz
        grads_bias = torch.mean(softmax_probs - one_hot_labels, dim=0)
