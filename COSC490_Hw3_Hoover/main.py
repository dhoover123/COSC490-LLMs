...
def explore_mlp_activations(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for activation, activation_names in zip(activations,activation_names):
        train_config = EasyDict({
            'batch_size': batch_size,
            'lr': lr,
            'num_epochs': num_epochs,
            'hidden_dims': hidden_dims,
            'save_path': f'model_activation_{activation_names}.pth',
            'embeddings': embedding_type,
            'num_classes': num_classes,
            'activation': activation,
        })
        epoch_train_losses, _, epoch_dev_loss, epoch_dev_accs, _, _ = run_mlp(train_config, embeddings, dev_dataset, train_dataset, test_dataset)

        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_activation_{activation_names}_loss.png")
...
def explore_mlp_learning_rates(dev_d: Dict[str, List[Union[str, int]]],
    ...
    for lr, lr_name in zip(lrs,lrs_names):
        ...
        all_emb_epoch_dev_accs.append(epoch_dev_accs)
        all_emb_epoch_dev_losses.append(epoch_dev_loss)
        visualize_epochs(epoch_train_losses, epoch_dev_loss, f"mlp_lr_{lr_name}_loss.png")
