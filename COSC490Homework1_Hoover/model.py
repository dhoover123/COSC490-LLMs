...
def evaluate(
    ...
    eval_loss, eval_acc = (
        np.array(eval_losses).mean(),
        np.array(eval_accs).mean(),
    )
...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        if dev_acc > best_acc:
            best_acc = dev_acc
            print(
                f"{'-' * 10} Epoch {epoch}: Best Acc so far: {best_acc} {'-' * 10}"
            )
            if save_path:
                print(
...
def visualize_configs(
    ...
    plt.clf()
    for config_epoch_stats, config_name in zip(
        all_config_epoch_stats, config_names
    ):
...
def run(
    ...
    print(
        f"{'-' * 10} Load Pre-trained Embeddings: {config.embeddings} {'-' * 10}"
    )
    ...
    dev_dataloader = create_dataloader(
        dev_dataset, config.batch_size, shuffle=False
    )
