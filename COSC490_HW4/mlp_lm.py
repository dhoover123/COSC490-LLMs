...
class NPLMFirstBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        transformed = torch.tanh(self.linear(concat_embeds))

        # apply layer normalization
        normed = self.layer_norm(transformed)
        # apply dropout
        final_embeds = self.dropout(normed)
        # your code ends here
...
class NPLMBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        residual = transformed + inputs

        # apply layer normalization
        normed = self.layer_norm(residual)

        # apply dropout
        final_inputs = self.dropout(normed)
        # your code ends here
...
class NPLMFinalBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        log_probs = F.log_softmax(transformed,dim = -1)
        # your code ends here
